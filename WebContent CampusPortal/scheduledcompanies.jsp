	...
	<div class="row">
		...
		<article class="col-md-8 maincontent">
			<header class="page-header">
				<h1 class="page-title">Scheduled Companies</h1>
			</header>
		   ...
		   <table class="table" >
			  <tbody style="width:10%;">
			  	<c:forEach var="company" items="${command.companyList}">
					...
					<tr >
						<td colspan="2"><h3><b><a href="${pageContext.request.contextPath}/getschedulecomapny/${company.name}">${company.name}</a></b></h3></td>
						
					</tr>
					
					<tr>
						<td colspan="2">Schedule Date : ${company.date} </td>
					</tr>
					<tr>
						<td>Reporting Time : ${company.timeofreporting}</td>
						<td>Place of Posting : ${company.placeofposting}</td>
					</tr>
				  ...
				  <spring:url value="/removeschedulecompany/${company.name}" var="applyurl" />
						<td align="right" colspan="2"><button onclick="location.href='${applyurl}'" class="btn btn-action" type="submit">Remove Schedule Company</button></td>
